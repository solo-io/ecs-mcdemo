...
for NU in 1 2; do
  # Delete ECS cluster
  echo "Deleting ECS cluster 'ecs-$CLUSTER_NAME-$NU'..."
  aws ecs delete-cluster --cluster ecs-$CLUSTER_NAME-$NU > /dev/null

  # Check if cluster deletion was successful
  if [ $? -ne 0 ]; then
    echo "Error: Failed to delete ECS cluster 'ecs-$CLUSTER_NAME-$NU'."
    exit 1
  else
    echo "ECS cluster 'ecs-$CLUSTER_NAME-$NU' has been deleted."
  fi
done
